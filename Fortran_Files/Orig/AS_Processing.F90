...
!* FIND INITIAL LOCATION OF 3 ELEVATIONS TO MONITOR AND REFERENCE ELEVATION
	...
	Implicit NONE
	Integer,intent(out):: IELV(3)
	Real(8),intent(in):: ELV(3),REFELV
	Real(8),intent(out):: XELVI(3),XELVR(3),XREFELV
	Integer,intent(out):: NRMAX(3)
	...
	IELV(1:3)=1
	IF(ELV(2)==ELV(1)) IELV(2)=0  
	IF(ELV(3)==ELV(2).OR.ELV(3)==ELV(1)) IELV(3)=0
	DO I=1,3
		IF(IELV(I)==0) Cycle
		CycleLoop=.false.
		DO J=NDX,2,-1
		...
		End Do
		if(CycleLoop) Cycle
		Write(TmpStr,'(" ERROR: THE ELEVATION: ",F0.2," ",A," DOES NOT EXIST&
			& ON THE INITIAL PROFILE.")') -ELV(I)/FACTL,Trim(LUNITS)
	...
	if(ErrOcc)Then
		Write(TmpStr,'(" ERROR: THE ELEVATION: ",F0.2," ",A," DOES NOT EXIST&
			& ON THE INITIAL PROFILE.")') -REFELV/FACTL,Trim(LUNITS)
		Call WriUFLOG(TmpStr)
		Return
	End If
	return
